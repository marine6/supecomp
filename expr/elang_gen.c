...
struct expression *make_terms(struct list *l) {
    struct expression *res = make_expr(l->elt);
    l = l->next;

    while (l != NULL) {
        struct ast_node *node = l->elt;
        res = make_expr_binop(binop_of_ast(node->tag), res, make_expr(l->next->elt));
        l = l->next; // skip next factor
        l = l->next; // next
    }
    return res;
}
...
struct instruction *make_instr(struct ast_node *ast) {
    ...
    switch (ast->tag) {
        ...
        case AST_IASSIGN:
            return make_instr_assign(
                    string_of_string_leaf(list_nth(ast->children, 0)),
        ...
        case AST_IIFTHENELSE:
            return make_instr_ifthenelse(
                    make_expr(list_nth(ast->children, 0)),
